...
func main() {
	log.Println("Starting Comprehensive Cache Performance Test...")
...
func testBasicOperationsPerformance(ctx context.Context, cache cacheTypes.Cache) error {
	...
	operations := []struct {
	...
	}{
		{
			...
			test: func() error {
				return benchmarkSetOperations(ctx, cache, 1000)
			},
		},
		{
			name: "GET_PERFORMANCE",
			test: func() error {
				return benchmarkGetOperations(ctx, cache, 1000)
			},
		},
		{
			name: "DELETE_PERFORMANCE", 
			test: func() error {
				return benchmarkDeleteOperations(ctx, cache, 1000)
			},
		},
	}
...
func testConcurrentLoadPerformance(ctx context.Context, cache cacheTypes.Cache) error {
	...
	testConfigs := []LoadTestConfig{
		{Concurrency: 10, TotalRequests: 1000, RequestTimeout: 1 * time.Second, DataSize: 100, TTL: 5 * time.Minute},
		{Concurrency: 50, TotalRequests: 5000, RequestTimeout: 1 * time.Second, DataSize: 100, TTL: 5 * time.Minute},
		{Concurrency: 100, TotalRequests: 10000, RequestTimeout: 1 * time.Second, DataSize: 100, TTL: 5 * time.Minute},
	}
...
func testCacheHitMissRatio(ctx context.Context, cache cacheTypes.Cache) error {
	...
	scenarios := []struct {
		...
		{"High Hit Ratio", 0.9, 1000},
		{"Medium Hit Ratio", 0.5, 1000},
		{"Low Hit Ratio", 0.1, 1000},
	}
...
func testMemoryUsage(ctx context.Context, cache cacheTypes.Cache) error {
	...
	testCases := []struct {
		...
		{1000, 100, "Small entries"},
		{500, 1024, "Medium entries"},
		{100, 10240, "Large entries"},
	}
